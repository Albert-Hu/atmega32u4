...
/** DFU attribute mask, indicating that the DFU device will detach and re-attach
 * when a DFU_DETACH command is issued, rather than the host issuing a USB
 * Reset.
 */
#define ATTR_WILL_DETATCH (1 << 3)

/** DFU attribute mask, indicating that the DFU device can communicate during
 * the manifestation phase (memory programming phase).
 */
#define ATTR_MANEFESTATION_TOLLERANT (1 << 2)

/** DFU attribute mask, indicating that the DFU device can accept DFU_UPLOAD
 * requests to send data from the device to the host.
 */
#define ATTR_CAN_UPLOAD (1 << 1)

/** DFU attribute mask, indicating that the DFU device can accept DFU_DNLOAD
 * requests to send data from the host to the device.
 */
...
/* Type Defines: */
/** Type define for a DFU class function descriptor. This descriptor gives DFU
 * class information to the host when read, indicating the DFU device's
 * capabilities.
 */
typedef struct {
